...
func ReadEmbeddingsCsv(file string, is_hyperbolic bool) *Corpus {
	...
	c.embeddings = make([]int32, c.params.EmbeddingSlots*c.params.NumDocs)
	...
	if is_hyperbolic {
		fmt.Printf("Use Hyperbolic Embeddings!\n")
		c.betas = make([]int32, c.params.NumDocs)
	}
	...
	for at := uint(0); ; at++ {
		...
		if slots > 0 {
			if is_hyperbolic {
				u, err := strconv.Atoi(row[c.params.EmbeddingSlots+2])
				if err != nil {
					panic("Error parsing CSV embeddings")
				}
				c.betas[at] = embeddings.Clamp(u, c.params.SlotBits)
			}
		}
...
func ReadOffsetsCsv(file string) *Corpus {
	f := utils.OpenFile(file)
	defer f.Close()
	...
	for at := uint(0); ; at++ {
		...
		u, err := strconv.Atoi(row[c.params.EmbeddingSlots+3])
		if err != nil {
			panic("Error parsing CSV embeddings")
		}
		//c.embeddingsClusterMap[at] = offset
	...
	if uint64(len(c.betas)) != c.params.NumDocs {
		panic("Should not happen!")
	}
...
func ReadEmbeddingsTxt(clusterStart, clusterStop int, conf *config.Config) *Corpus {
	...
	for cluster := clusterStart; cluster < clusterStop; cluster++ {
		...
		for scanner.Scan() {
			...
			emb := make([]int32, c.params.EmbeddingSlots)
