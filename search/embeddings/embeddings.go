...
func RandomEmbedding(length, mod uint64) []int32 {
	vals := make([]int32, length)

	for i := uint64(0); i < length; i++ {
		vals[i] = int32(rand.Intn(int(mod)))
		if rand.Intn(2) == 1 {
			vals[i] *= -1
		}
	}
...
func ShrinkPrecision(emb []int, slot_bits uint64) []int32 {
	arr := make([]int32, len(emb))
	for i, v := range emb {
...
func InnerProduct(v1, v2 []int32) int {
	if len(v1) != len(v2) {
...
func Clamp(val int, slotBits uint64) int32 {
